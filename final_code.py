...
#dataset
DATASET_PATH = "/Users/bernardaandradefelix/AI/Final_Foundation_dataset.csv"
...
# --- Lazy MediaPipe Face Detector Initialization ---
_face_detector = None
...
# Define update and capture after all functions are declared
...
def capture_image():
    ...
    skin_hex = bgr_to_hex(extract_dominant_color(crop))
    cap.release()
    root.destroy()
    show_preview(crop, skin_hex)

# Now place the capture button after capture_image is defined
...
def show_preview(crop_img, skin_hex):
    ...
    frame = tk.Frame(p, bg=PRIMARY, bd=5); frame.pack(padx=20,pady=10)
    rgb = cv2.cvtColor(crop_img, cv2.COLOR_BGR2RGB)
    im = ImageTk.PhotoImage(Image.fromarray(rgb)); lbl = Label(frame, image=im); lbl.image=im; lbl.pack()
...
def open_results(skin_hex):
    ...
    frame = tk.Frame(w, bg=BG); frame.pack(pady=10)
    ...
    mf = tk.Frame(w, bg=BG); mf.pack(fill="both",expand=True,padx=20,pady=10)
    c = tk.Canvas(mf, bg=BG); sb = tk.Scrollbar(mf,orient="vertical",command=c.yview)
    sf = tk.Frame(c, bg=BG)
